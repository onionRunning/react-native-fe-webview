...
const ExWebView = React.forwardRef((props: Props, ref) => {
  ...
  const onPostMessage = (payload: {type: string, params: any}) => {
    // console.warn('我执行了么222222', payload)
  ...
  const closeKeyBoard = () => {
    if (webviewRef.current) {
      UIManager?.dispatchViewManagerCommand?.(
        findNodeHandle?.(webviewRef.current),
        UIManager.getViewManagerConfig('MaxWebView').Commands.closeKeyboard!,
        [],
      )
    }
  }
  ...
  const openKeyBoard = () => {
    UIManager.dispatchViewManagerCommand(
      findNodeHandle(webviewRef.current),
      UIManager.getViewManagerConfig('MaxWebView').Commands.openKeyboard!,
      [],
    )
  }
